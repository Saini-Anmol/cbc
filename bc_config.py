...
# ── MID-MONTH BUILDING CARRY-IN: rolling N-day SKU SET per machine ────────────
# For a mid-month PLAN_START, each building machine is started on a SKU it ACTUALLY
# built in the N days immediately BEFORE the start date — not one frozen SKU.
# Source = a FULL-MONTH baseline plan (1st -> month end) run with NO production
# deduction; from it we read, per machine, the distinct SKUs built on the N days
# before PLAN_START. On day 1 the machine picks the SKU from that set with the best
# live need (curing draw / GT + carcass inventory / deficit) and is charged NO
...
MIDMONTH_BUILDING_SEED_FILE = ""    # superseded by the N-day SET below; set a path to force one SKU/machine
...
PLANT_SET_LOCK = True   # ← PLANT-SET LOCK (plant-first, spare→others): from day 3+, each GT machine (VMI/BJ/Unistage/PS + Stage-2; Stage-1 auto-follows Stage-2) is COMMITTED to the SKU SET the plant ran on it in Days 1-2 (from PLANT_2DAY_SCHEDULE_FILE). Its plant-set SKUs are built FIRST (Phase A/B) and never abandoned; a NON-plant SKU is allowed on that machine ONLY as SPARE — when no plant-set SKU still needs building that shift (all their per-shift draw is met) — plus a SOLE-BUILDER override (a SKU whose only allowable machines are all plant-locked elsewhere isn't orphaned). Once every SKU in the machine's plant set is demand-complete, the machine is RELEASED to its full allowable matrix. Env PLANT_SET_LOCK=0 reverts bit-for-bit.
CO_STARVE_BYPASS = True   # ← STARVING-PRESS CO BYPASS: allow a SAME-INCH building changeover past the 30%-of-remaining CO-cost guard when the target SKU's curing presses are RUNNING but STARVED (live draw, no GT on hand/built, demand remaining) — else a machine with a half-used shift idles its residual minutes rather than pay a cheap CO to feed its own empty presses (e.g. 7503→TUHL0). Same-inch + demand/curable-capped → no waste-GT risk. Env CO_STARVE_BYPASS=0 reverts.
COLDSTART_BOOTSTRAP = True   # ← COLD-START BOOTSTRAP (curing_consumption_dynamic.py): a demand SKU with eligible presses + an eligible building machine but 0 presses committed all month never enters the acquisition pool (its presses are HOLD-locked on other SKUs) → curing draw 0 → building _defc≤0 → 0 GT → press never seeded (e.g. 8501→1D25215Z13008QXPC0). Once per SKU, directly CO one n-1-protected donor press onto it to seed the draw; building then follows + PRESS_STABLE holds it. Needs the SKU to actually be buildable (a Stage-2 SKU needs a Stage-1 carcass source, else the seeded press starves). Env COLDSTART_BOOTSTRAP=0 reverts.
# ══════════════════════════════════════════════════════════════════════════════
