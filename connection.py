...
def _load_cycle_times(engine) -> dict:
    try:
        ...
        df["sku"] = df["sku"].astype(str).str.strip()
        # effective CT = raw / PRESS_EFFICIENCY (0.95 for 4-digit presses) — consistent with
        # ConsumptionETL.load_cycle_times; NOT a hardcoded value.
        df["ct"]  = np.round((pd.to_numeric(df["raw_ct"], errors="coerce") + 0)
                             / ConsumptionConfig.PRESS_EFFICIENCY, 1)
